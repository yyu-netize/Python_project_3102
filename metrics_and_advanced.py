"""
metrics_and_advanced.py

1. 加载清洗后的 PvZ QA 数据集（qa_dataset_clean.json）
2. 文本指标：
   - Exact Match (EM)
   - Token-level F1
3. 检索指标：
...
4. 答案质量指标：
   - 对 RAG 生成回答计算 EM / F1
5. LLM-as-Judge：
   - 对模型回答做 faithfulness / relevance 等主观评分（至少 30 条）

依赖：
- 复用 generator.py 里的 UltimateRAGWithGenerator, client, MODEL_NAME
- 复用你自己的多轮模块里的 MultiTurnRAGChat（只要文件名对上即可）
"""
...
# === 根据你的实际文件名修改这里 ===
# 假设你把 cell1-4 存在 multi_turn_chat.py 里
from multi_turn_chat import MultiTurnRAGChat, TurnRecord

# 从 generator 导入 RAG 封装 & Qwen 客户端
...
def exact_match(pred: str, gold: str) -> float:
    """
    Exact Match：归一化后完全相同记作 1.0ll否则 0.0
    """
...
def build_rag_handles() -> RAGHandles:
    """
    构建一个 UltimateRAGWithGenerator 实例，并取出内部的 core RAG。
    这样既可以：
    - 用 rag_core.retrieve_xxx 做检索指标
    - 用 rag_gen.search(...) / MultiTurnRAGChat 做答案生成
    """
...
def get_retrieved_ids_for_query(
    ...
    """
    给定一个 query，从 RAG core 里拉出 top_k 文档的 id 列表。
    - retrieve_mode: 'dense' / 'bm25' / 'hybrid'
    - use_rerank: 是否使用 cross-encoder 做重排序（Advanced 2 ）
    """
    ...
    # 2) 如果不开 rerank：直接取前 top_k
    if not use_rerank:
        return [c["id"] for c in candidates[:top_k]]

    # 3) 开启 rerank：用 cross-encoder 对 candidates 排序，再取前 top_k
...
def eval_retrieval_metrics(
    ...
    """
    对检索器做评估：计算 Recall@k 和 MRR。
    - retrieve_mode: dense / bm25 / hybrid
    - use_rerank: False 表示只评估“召回排序”，True 表示评估“重排后排序”
      （可以用来做 Advanced 2 的 ablation: with vs without rerank）
...
def run_retrieval_ablation(
    ...
    一次性跑完：
      ...
      rerank = [False, True]
    共 8 组组合的 Recall@k / MRR，对比检索/重排序效果。
...
def eval_answer_quality_single_turn(
    ...
    """
    用 MultiTurnRAGChat 做“单轮评估”：
    - 每一条 QA 都 reset 一次对话历史
    - enable_rewrite=True/False 用于对比 condense question 的效果
    - 内部会调用 UltimateRAGWithGenerator.search(...) 来生成答案

    返回：
    - avg_em
    - avg_f1
    - result_path (详细结果 JSON，用于后续 LLM-as-Judge)
    """
    qa_items = load_qa_dataset(qa_path)
    if sample_size is not None and 0 < sample_size < len(qa_items):
        random.seed(random_seed)
    ...
    # MultiTurnRAGChat 里会自己初始化一个 UltimateRAGWithGenerator
    ...
    for idx, item in enumerate(qa_items, start=1):
        ...
        # 拿出最后一轮 TurnRecord（里面有 rewritten query）
...
def llm_judge_answers(
    ...
    """
    LLM-as-Judge: evaluate model answers in an eval_answers_*.json file.

    - eval_result_path: JSON produced by eval_answer_quality_single_turn
      (e.g., eval_answers_hybrid_rerankTrue_rewriteTrue.json)
    - sample_size: number of samples to judge (>= 30 for coursework requirement)
    """
    # 1. Load eval result file
    ...
    for idx, rec in enumerate(records, start=1):
        # ✅ 兼容两种字段命名：gold/gold_answer, pred/pred_answer
...
def eval_answer_quality_single_turn(
    qa_path: str = "qa_dataset_clean.json",
    retrieve_mode: str = "hybrid",
    use_rerank: bool = True,      # 暂时只用于记录和打印，真正是否 rerank 由内部 RAG 配置决定
    enable_rewrite: bool = False,
    sample_size: int = 100,
    random_seed: int = 42,
) -> str:
    ...
    单轮问答评估：
    - 对每个问题 reset 会话
    - 按给定的 retrieve_mode / rewrite 来跑回答（use_rerank 目前只是标记）
    - 输出 EM / F1 / ROUGE-L
    - 保存详细结果 JSON（可供 LLM-as-Judge 用）
    ...
    # 这里不再调用 build_rag_handles()，因为 MultiTurnRAGChat 内部自己管理 RAG 实例
    # handles = build_rag_handles()
    ...
    # 单轮 → 每题前初始化多轮系统
    # 注意：MultiTurnRAGChat 内部会初始化 UltimateRAGWithGenerator 和 RAG core
    chat = MultiTurnRAGChat(
        enable_rewrite=enable_rewrite,
        default_retrieve_mode=retrieve_mode,
    ...
    for idx, item in enumerate(qa_items, 1):
        ...
        pred = chat.ask(q)  # 利用 rewrite + RAG + generator 搜答案
...
def main():
    ...
    # 支持 --mode 和 --retrieve-mode 两种写法，最终都存在 args.retrieve_mode 里
    p_ret.add_argument(
        "--mode", "--retrieve-mode",
        dest="retrieve_mode",
        type=str,
    ...
    # 支持 --rewrite / --enable-rewrite 两种写法
    p_ans.add_argument(
