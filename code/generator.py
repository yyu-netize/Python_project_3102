...
# --- CONFIGURATION ---
SILICONFLOW_API_KEY = "sk-qxocptxsyigkhfruvqjevqjngthhmzdwetgonkditizarmue"  # SiliconFlow API key
...
"""
    ...
    def search(self, query, retrieve_mode, rerank_mode, prompt_mode, message_mode):
        """
...
if __name__ == "__main__":
    ...
    # test cases
    rag_with_generator.search("Which plant can slow down zombies?", retrieve_mode="hybrid", rerank_mode=True, prompt_mode="instruction", message_mode="with_system")
    rag_with_generator.search("What is the sun cost of Peashooter?", retrieve_mode="dense", rerank_mode=True, prompt_mode="instruction", message_mode="with_system")
    rag_with_generator.search("Difference between Cherry Bomb and Jalapeno", retrieve_mode="bm25", rerank_mode=True, prompt_mode="instruction", message_mode="with_system")
'''
def main():
    """
    命令行交互入口：只在直接运行 generator.py 时使用。
    Notebook 等其他模块 import 本文件时不会自动执行。
    """
    rag_with_generator = UltimateRAGWithGenerator()
    while True:
        print("\n=== RAG Query System ===")
        print("Input format example: Your question | retrieve_mode | rerank_mode | prompt_mode | message_mode")
        print("Parameter description:")
        print("- retrieve_mode: hybrid / dense / bm25 / hyde (default: hybrid)")
        print("- rerank_mode: True / False (default: True)")
        ...
        if len(parts) >= 3 and parts[2]:
            rerank_mode = True if parts[2].lower() == "true" else False
        if len(parts) >= 4 and parts[3]:
            prompt_mode = parts[3]
        if len(parts) >= 5 and parts[4]:
            message_mode = parts[4]
        ...
        if rerank_mode not in valid_rerank_modes:
            print(f"Invalid rerank_mode: {rerank_mode}, using default value True")
            rerank_mode = True
        ...
        # Execute query
        if query:
            print(f"\nExecuting query - retrieve_mode: {retrieve_mode}, rerank_mode: {rerank_mode}, prompt_mode: {prompt_mode}, message_mode: {message_mode}")
