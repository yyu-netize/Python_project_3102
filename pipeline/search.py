...
MODEL_NAME = "bge-m3" # Embedding model
RERANKER_MODEL_NAME = "bge-reranker-v2-m3" # Reranker
...
class UltimateRAG:
    ...
    def get_query_embedding(self, query):
        ...
        """
        task_instruction = "" \
        "Retrieve detailed attributes, stats, and strategies for Plants vs. Zombies game entities."
...
Directly generate a factual, CONCISE document that would answer the following question.
...
"""
    ...
    def retrieve_hybrid(self, query, top_k=30):
        """
        Hybrid retriever: get merged candidates from both Vector and BM25
        """
        ...
        # Put Vector results into candidates 
        if vec_results['ids']:
            for i, doc_id in enumerate(vec_results['ids'][0]):
                candidates[doc_id] = {
                    'id': doc_id,
    ...
    def search(self, query, retrieve_mode, rerank_mode):
        print(f"\n🔎 Query: {query}")
        ...
        if (retrieve_mode == "hybrid"):
            ...
            print(f" - Dense Retriever + BM25 find {len(candidates)} candidates.")
...
def main():
    while True:
        print("\n=== RAG Query System ===")
        print("Input format example: Your question | retrieve_mode | rerank_mode")
        ...
        # Execute query
        if query:
            print(f"\nExecuting query - retrieve_mode: {retrieve_mode}, rerank_mode: {rerank_mode}")
            rag.search(
                query, 
...
if __name__ == "__main__":
    ...
    # Test cases
    rag.search("Which plant can slow down zombies?", retrieve_mode="hybrid", rerank_mode=True)
    rag.search("What is the sun cost of Peashooter?", retrieve_mode="dense", rerank_mode=True)
    rag.search("Difference between Cherry Bomb and Jalapeno", retrieve_mode="hyde", rerank_mode=True)
